...
const AVAILABLE_MODELS: ModelOption[] = [
  { name: 'GPT-OSS 120B', desc: 'Groq LPU · High precision reasoning (Default)' },
  { name: 'GPT-OSS 20B', desc: 'Groq LPU · Balanced fast inference' },
  { name: 'Qwen 3.8 27B', desc: 'Groq LPU · Advanced multi-lingual & technical' },
  { name: 'Qwen 3.6 27B', desc: 'Groq LPU · Low-latency generation' },
  { name: 'Groq Compound', desc: 'Groq LPU · Agentic multi-tool compound model' },
];
        ...
        <>
          ...
          <div className="absolute bottom-full mb-1 left-2 sm:left-8 z-10 w-full max-w-[calc(100vw-32px)] sm:max-w-sm overflow-hidden pointer-events-none p-0.5">
            <div className={`${isMentionClosing ? 'pure-slide-down' : 'pure-slide-up'} pointer-events-auto w-full max-h-72 overflow-y-auto p-1.5 bg-[var(--bg-modal)] border border-[var(--border-color)] rounded-2xl text-xs`}>
              <div className="flex items-center justify-between px-3 py-1.5 text-[11px] font-bold text-[var(--text-dark)] uppercase tracking-wider select-none border-b border-[var(--border-color)] mb-1">
                <span className="flex items-center gap-1.5 text-[var(--text-main)]">
                  ...
                  {mentionQuery && <span className="lowercase font-mono text-[var(--text-muted)] font-normal">&ldquo;{mentionQuery}&rdquo;</span>}
                </span>
                <span className="font-mono text-[11px] text-[var(--text-muted)]">Tab / ↵</span>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between px-3 py-2 rounded-xl cursor-pointer transition-all duration-150 ${
                        isSelected
                          ? 'bg-[var(--accent-subtle)] shadow-xs ring-1 ring-[var(--accent-primary)]/40 font-medium'
                    ...
                    >
                      <div className="flex items-center gap-2.5 truncate pr-2">
                        ...
                        <div className="truncate">
                          <div className="font-semibold text-[13px] text-[var(--text-main)] truncate">{doc.filename}</div>
                          <div className="text-[10.5px] text-[var(--text-muted)] font-mono">
                            {doc.pages} pages · {doc.size_mb} MB
                          </div>
                        </div>
                      </div>
            ...
            <div className="absolute bottom-full mb-1 left-2 z-30 w-64 overflow-hidden pointer-events-none p-0.5">
              <div className={`${isPlusClosing ? 'pure-slide-down' : 'pure-slide-up'} pointer-events-auto w-full py-1.5 px-1 bg-[var(--bg-modal)] border border-[var(--border-color)] rounded-2xl backdrop-blur-2xl text-xs select-none flex flex-col gap-0.5`}>
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (e?.name !== 'NotAllowedError') {
                        showToast("Screenshot cancelled or failed");
                      }
                    }
                  }}
                >
                ...
                <div 
                  ...
                  onClick={() => {
                    closePlusMenu();
                    const githubBtn = document.querySelector('[data-action="github-sync"]') as HTMLButtonElement | null;
                    if (githubBtn) {
                      githubBtn.click();
                    } else {
                      showToast("Navigate to Knowledge Vault to sync GitHub repos");
                    }
                  }}
                >
                  <div className="flex items-center gap-2.5">
                ...
                <div 
                  ...
                  onClick={() => {
                    closePlusMenu();
                    showToast("Skills & custom instructions active");
                  }}
                >
                  <div className="flex items-center gap-2.5">
                ...
                <div 
                  ...
                  onClick={() => {
                    closePlusMenu();
                    showToast("Connectors: Qdrant, Supabase, GitHub connected");
                  }}
                >
                  <div className="flex items-center gap-2.5">
                ...
                <div 
                  ...
                  onClick={() => {
                    closePlusMenu();
                    showToast("Plugins & tools library");
                  }}
                >
                  <div className="flex items-center gap-2.5">
                    <Plug size={16} className="text-[var(--text-muted)]" />
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <Globe size={16} className={webSearchEnabled ? "text-[var(--accent-primary)]" : "text-[var(--text-muted)]"} />
                    <span className={webSearchEnabled ? "font-medium text-[var(--accent-primary)]" : ""}>Web search</span>
                  </div>
                  {webSearchEnabled && (
                    <Check size={16} className="text-[var(--accent-primary)] flex-shrink-0" />
                  )}
                </div>
