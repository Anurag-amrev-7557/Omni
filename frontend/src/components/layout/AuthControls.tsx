...
export const AuthControls: React.FC<AuthControlsProps> = ({ onOpenAuth }) => {
  ...
  return (
    <button 
      onClick={onOpenAuth} 
      className="flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] text-[var(--text-main)] hover:bg-[var(--bg-hover)] transition-all cursor-pointer shadow-2xs"
    >
      Sign in
    </button>
  );
