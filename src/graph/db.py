...
def _user_clause(uid: str) -> tuple[str, tuple]:
    """Returns a WHERE clause fragment and params that correctly scopes to the given user,
    including the DEFAULT_LOCAL_USER sentinel for local/dev mode queries.

    Returns (clause_str, params_tuple).
    """
    if is_default_or_local_user(uid):
        return (
            "(user_id = ? OR user_id = ? OR user_id = ?)",
            (uid, DEFAULT_LOCAL_USER, "default_user"),
        )
    return "user_id = ?", (uid,)
...
def sync_and_prune_graph(user_id: str | None = None, active_filenames: list[str] | None = None):
    ...
    # 1. Delete relations for documents no longer in active_set
    ...
    # 4. If nothing remains, drop communities
