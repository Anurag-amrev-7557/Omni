...
def init_chat_db():
    ...
    try:
        ...
        cursor.execute("ALTER TABLE sessions ADD COLUMN user_id TEXT NOT NULL DEFAULT '10d2f529-3fae-4a29-9a5e-312876700ff9'")
...
def create_session(title: str = "New Chat", user_id: str = "10d2f529-3fae-4a29-9a5e-312876700ff9") -> str:
    session_id = str(uuid.uuid4())
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO sessions (session_id, title, user_id, created_at) VALUES (?, ?, ?, ?)",
...
def delete_user_sessions(user_id: str):
    """Deletes all sessions and related messages for a given user or guest."""
    if not user_id:
        return
    conn = get_db_connection()
    cursor = conn.cursor()
