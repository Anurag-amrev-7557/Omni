...
def init_chat_db():
    try:
        ...
        print(f"[Warning] Could not initialize chat database tables: {exc}")
...
def create_session(title: str = "New Chat") -> str:
    ...
    try:
        ...
        print(f"[Warning] Failed to create session in DB: {exc}")
    return session_id
...
def get_all_sessions() -> list[dict]:
    try:
        ...
        print(f"[Warning] Failed to fetch sessions from DB: {exc}")
        return []
...
def update_session_title(session_id: str, title: str):
    try:
        ...
        print(f"[Warning] Failed to update session title in DB: {exc}")

def add_message(session_id: str, role: str, content: str, contexts: list[dict] | None = None, session_title: str | None = None):
    try:
        user_id = get_current_user()
        with connection() as conn, conn.cursor() as cur:
            cur.execute("SELECT title FROM chat_sessions WHERE session_id=%s AND user_id=%s", (session_id, user_id))
            ...
            if existing is None:
                ...
                cur.execute("""
                    ...
                    VALUES (%s, %s, %s) 
                    ON CONFLICT (session_id) DO UPDATE SET title = EXCLUDED.title WHERE chat_sessions.title = 'New Chat' OR EXCLUDED.title != 'New Chat'
                """, (session_id, user_id, title))
            elif session_title and (existing.get("title") == "New Chat" or session_title != "New Chat"):
                cur.execute("UPDATE chat_sessions SET title=%s WHERE session_id=%s AND user_id=%s", (session_title, session_id, user_id))
        ...
        print(f"[Warning] Failed to persist chat message in DB: {exc}")
...
def get_session_messages(session_id: str) -> list[dict]:
    try:
        ...
        print(f"[Warning] Failed to get session messages from DB: {exc}")
        return []
...
def delete_session(session_id: str):
    try:
        ...
        print(f"[Warning] Failed to delete session from DB: {exc}")
